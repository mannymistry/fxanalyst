...
This is an R Markdown document. This document is automated and meant to provide meaningful start of day report for and FX an Collateral Analyst. This was created over a couple of hours and is intended to demonstrate my ability to rapidly prototype reports using R and freely available data. 
...
Using hourly tick data for the FX pairs from IEX the Analyst would monitor these prices at a granularity he/she sees fit. The chart and the associated code is meant as a jumping off point to preform deeper dives and add additional explanatory variables to regress against. 

#### Correlation Chart

Using the daily returns of the FX prices correlation is calculated and plotted. The Analyst would use this graph to monitor the correlation. Some pairs are more obviously correlated and if correlation on those pairs start to move in the opposite direction an opportunity could present itself. 
